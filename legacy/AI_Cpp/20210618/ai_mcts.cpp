...
void mcts_expand(const unsigned long long p, const unsigned long long o, const unsigned long long mobility){
    unsigned long long np, no;
    int i;
    pair<unsigned long long, unsigned long long> grid;
    search_param.win_rate[make_pair(p, o)].isleaf = false;
    ...
    for (i = 0; i < hw2; ++i){
        if (1 & (mobility >> i)){
            ...
            grid.first = no;
            grid.second = np;
            search_param.win_rate[grid].isleaf = true;
            search_param.win_rate[grid].n = 0;
            search_param.win_rate[grid].m = 0;
            //search_param.win_rate[grid].val = 0.0;
            //search_param.win_rate[grid].val = -bias2 * evaluate(no, np, n_canput, calc_open(np | no, np ^ p));
        }
    }
}

int mcts_evaluate(const unsigned long long p, const unsigned long long o, const int skip_cnt, const unsigned long long mobility){
    if (skip_cnt == 2)
        return -end_game(p, o);
    //unsigned long long mobility = check_mobility(p, o);
    int n_canput = pop_count_ull(mobility);
    if (n_canput == 0)
        return -mcts_evaluate(o, p, skip_cnt + 1, check_mobility(o, p));
    unsigned long long np, no;
    int i;
    /*
    ...
    for (i = 0; i < hw2; ++i){
        if (1 & (mobility >> i)){
            ...
            tmp.p = np;
            tmp.o = no;
            tmp.m = check_mobility(no, np);
            //tmp.priority = -evaluate(no, np, n_canput, calc_open(np | no, np ^ p)) + myrandom() * 0.2 - 0.1;
            lst.push_back(tmp);
        }
    }
    i = randint(0, n_canput - 1);
    return -mcts_evaluate(lst[i].o, lst[i].p, 0, lst[i].m);
    */
    double max_val = -inf;
    double val;
    unsigned long long ap, ao;
    unsigned long long am, n_mobility;
    for (i = 0; i < hw2; i++){
        if (1 & (mobility >> i)){
            np = move(p, o, i);
            no = (np ^ o) & o;
            n_mobility = check_mobility(no, np);
            val = (n_canput - pop_count_ull(n_mobility)) + myrandom() * 10.0 - 5.0;
            if (val > max_val){
                max_val = val;
                ap = np;
                ao = no;
    ...
    return -mcts_evaluate(ao, ap, 0, am);
}
...
int mcts(const unsigned long long p, const unsigned long long o, const int skip_cnt){
    ...
    int n_canput = pop_count_ull(mobility);
    if (n_canput == 0){
        if (search_param.win_rate[grid_now].isleaf)
            res = -mcts_evaluate(o, p, skip_cnt + 1, check_mobility(o, p));
        else
            res = -mcts(o, p, skip_cnt + 1);
        search_param.win_rate[grid_now].m += res;
        return res;
    }
    if (search_param.win_rate[grid_now].n == expand_threshold)
        mcts_expand(p, o, mobility);
    if (search_param.win_rate[grid_now].isleaf)
        return mcts_evaluate(p, o, skip_cnt, mobility);
    pair<unsigned long long, unsigned long long> grid_new;
    unsigned long long np, no, ap, ao;
    int i;
    double max_priority = -inf, priority;
    for (i = 0; i < hw2; ++i){
        if (1 & (mobility >> i)){
            np = move(p, o, i);
            no = (np ^ o) & o;
            grid_new.first = no;
            grid_new.second = np;
            if (search_param.win_rate[grid_new].n)
                priority = (double)search_param.win_rate[grid_new].m / search_param.win_rate[grid_new].n
                    + bias * sqrt(log((double)search_param.win_rate[grid_now].n) / (double)search_param.win_rate[grid_new].n);
            else
                priority = inf;
...
int main(int argc, char* argv[]){
    ...
    while (true){
        ...
        mcts_expand(p, o, in_mobility);
        while (tim() - search_param.strt < search_param.tl){
            ++search_param.playout_cnt;
            mcts(p, o, 0);
        }
        cerr << "expanded: " << search_param.win_rate.size() << endl;
        max_score = -inf;
        for (i = 0; i < hw2; i++){
            if (1 & (in_mobility >> i)){
                np = move(p, o, i);
                ...
                grid.first = no;
                grid.second = np;
                score = search_param.win_rate[grid].n; //(double)search_param.win_rate[grid].m / max(1, search_param.win_rate[grid].n);
                //cerr << search_param.win_rate[grid].n << " " << score << "  ";
                cerr << (hw2 - i - 1) / hw << (hw2 - i - 1) % hw << " " << search_param.win_rate[grid].n << " " << (double)search_param.win_rate[grid].m / max(1, search_param.win_rate[grid].n) << endl;
                if (score > max_score){
                    max_score = score;
                    ansy = (hw2 - i - 1) / hw;
                    ansx = (hw2 - i - 1) % hw;
